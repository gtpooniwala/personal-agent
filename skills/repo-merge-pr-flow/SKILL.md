...
- If the current checkout is a managed slot, release that slot after the merge succeeds.
...
- Treat it as a managed slot only when `slot_id` matches `^(slot|dyn)-[0-9]{2}$`.
...
- If `slot_id` matches the managed-slot pattern, run:
